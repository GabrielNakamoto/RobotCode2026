...
public class RobotState {
	...
	public void addOdometryObservation(OdometryObservation observation) {
		final var robotDisplacement = Arrays.stream(observation.moduleDisplacements)
				.reduce(Translation2d.kZero, Translation2d::plus)
				.rotateBy(observation.gyroYaw);
		final Pose2d poseEstimate = new Pose2d(
				odometryPose.getTranslation().plus(robotDisplacement),
				observation.gyroYaw
		);
